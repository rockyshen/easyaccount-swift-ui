...
文档日期：2026-08-05  
状态：**待后端实现 API 层**（业务侧已具备解析能力；HTTP 契约需按本文落地）  
关联：现有 `POST /api/chat` SSE（见 `docs/sse-stream-resume-api.md`）；iOS 待命区已合入 `feature/ux-polish`
...
| 追加说明 | 待命后可继续在文字框输入 |
| 发送 | 文字 + 本地 JPEG 一并进入用户气泡；**当前网络层仍只把 `content` 文本 POST 到 `/api/chat`** |
...
| 项 | 当前（`feature/ux-polish`） | API 就绪后 |
|----|---------------------------|------------|
| 待命区 UI | ✅ 已有 | 保持 |
| 压缩 | ✅ JPEG ≤1600px | 保持，再上传 |
| `ChatOutbound` | `{ content }` | 增加 `attachmentIds: [String]` |
| `ChatSSEClient.stream` | 只 POST JSON content | 发送前先 upload；再带 ids |
| 用户气泡图 | 本地 `attachmentJPEGs`（不持久化） | 可继续本地展示；可选改存 `attachmentId`/`url` |
| 占位文案 `【图片】` | 纯图时用于过 `emptyContent` 校验 | 服务端允许空 content + ids 后可去掉 |
...
上传：`POST /api/chat/attachments`，`multipart/form-data` 字段名 `file`。
...
| 优先级 | 项 |
|--------|----|
| P0 | `POST /api/chat/attachments` + 鉴权 + 大小/MIME 校验 |
| P0 | `POST /api/chat` 接受 `attachmentIds`，接入已有图片解析业务，SSE 不变 |
| P0 | 开聊前校验附件归属与过期 |
| P1 | `GET /api/chat/attachments/{id}`（调试/回看） |
| P2 | `DELETE` 未引用附件；GC 任务 |

---

## 10. 联调联系

- 客户端分支：`feature/ux-polish`（附件待命区）  
- 相关文档：`docs/sse-stream-resume-api.md`  
- 本文路径：`docs/chat-attachments-api-backend-handoff.md`  

后端 API 按本文就绪后，通知 iOS 侧改 `ChatSSEClient` / `ChatOutbound` 并做联调；**完成前不要合入 `main` 发版依赖附件上传的构建。**
